 ...
 *
 * The kernel's mnt_ns_loop() loop-prevention check (used when the capture
 * helper bind-mounts /proc/$pid/ns/mnt) assumes mount namespace ids are
 * strictly globally ordered by creation time. As of Linux 6.18 that is no
 * longer true: ids are generated by a per-CPU batched counter, so a
 * namespace created later can receive a numerically smaller id than an
 * older one if the two allocations land on different CPUs. When that
 * happens the capture helper's bind mount fails with EINVAL for a namespace
 * that is otherwise perfectly fine.
 *
 * This must be called immediately after unshare(CLONE_NEWNS), before any
