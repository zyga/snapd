						    ...
						    gid_t saved_gid)
{
	...
	sc_explain("  - Root file system is %s", inv->rootfs_dir);
	...
	if (retval == ESRCH) {
		...
		sc_explain("    desired fstab "
			   "/var/lib/snapd/mount/snap.%s.fstab",
			   inv->snap_instance);
		sc_explain("    effective fstab " "/run/snapd/ns/snap.%s.fstab",
			   inv->snap_instance);
	...
	if (sc_cgroup_is_v2()) {
		sc_explain("  - Freezer cgroup v1: "
			   "unsupported, systemd in v2 mode");
	} else {
	...
	sc_explain("  - Setting PATH to %s", fixed_path);
	...
	for (i = 0; tmpd[i] != NULL; i++) {
		sc_explain("  - Setting %s to /tmp", tmpd[i]);
