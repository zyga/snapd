...
void sc_init_invocation(sc_invocation *inv, const struct sc_args *args, const char *__null_terminated snap_instance,
                        const char *__null_terminated snap_component) {
    ...
    char *__unsafe_indexable component_name = NULL;
     ...
     * tag. */
    const char *__null_terminated security_tag = sc_args_security_tag(args);
    if (!sc_security_tag_validate(security_tag, snap_instance,
                                  __unsafe_forge_null_terminated(const char *, component_name))) {
     ...
     * base. */
    const char *__null_terminated base_snap_name = sc_args_base_snap(args);
    ...
    const char *__null_terminated executable = sc_args_executable(args);
    if (executable == NULL) {
    ...
    inv->snap_name = sc_strdup(__unsafe_forge_null_terminated(const char *, &snap_name[0]));
    ...
    inv->rootfs_dir = sc_strdup(__unsafe_forge_null_terminated(const char *, &mount_point[0]));
...
void sc_cleanup_invocation(sc_invocation *__unsafe_indexable inv) {
    if (inv != NULL) {
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->snap_instance);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->snap_name);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->base_snap_name);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->orig_base_snap_name);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->security_tag);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->executable);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->rootfs_dir);
        sc_cleanup_string((char *__unsafe_indexable *)(void *)&inv->snap_component);
...
static char *__null_terminated read_homedirs_from_system_params(void) {
    ...
    while (getline(&line, &line_size, f) != -1) {
        if (sc_startswith(__unsafe_forge_null_terminated(const char *, line), "homedirs=")) {
            return sc_strdup(__unsafe_forge_null_terminated(const char *, line + (sizeof("homedirs=") - 1)));
        }
    }
    return NULL;
}
...
void sc_invocation_init_homedirs(sc_invocation *inv) {
    char *__unsafe_indexable config_line SC_CLEANUP(sc_cleanup_string) =
        __unsafe_forge_null_terminated(char *, read_homedirs_from_system_params());
    if (config_line == NULL) {
        return;
    }
     ...
     */
    int num_commas = 0;
    for (char *__unsafe_indexable c = config_line; *c != '\0'; c++) {
        if (*c == ',') {
            num_commas++;
        }
    }
    ...
    int current_index = 0;
    char *__unsafe_indexable homedir = strtok_r(config_line, ",\n", &buf_saveptr);
    while (homedir != NULL) {
        if (homedir[0] == '\0') {
            // Deal with the case of an empty homedir line (e.g "homedirs=")
            continue;
        }
        inv->homedirs[current_index++] = sc_strdup(__unsafe_forge_null_terminated(const char *, homedir));
