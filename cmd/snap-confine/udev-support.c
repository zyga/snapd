...
static void sc_udev_make_confined(int devices_allow_fd, int devices_deny_fd,
				  struct udev *udev,
				  struct udev_list_entry *assigned)
{
...
static void sc_udev_make_unconfined(int devices_allow_fd)
{
	dprintf(devices_allow_fd, "a *:* rwm");
}
...
void sc_setup_device_cgroup(const char *security_tag)
{
	...
	if (assigned != NULL) {
		 ...
		 *
		 * Create a cgroup v1 with a device controller that allows access to
		 * some common set of devices, the set of tagged devices and nothing
		 * else. */
		sc_udev_make_confined(fds.devices_allow_fd, fds.devices_deny_fd,
				      udev, assigned);
	} else {
		 ...
		 *
		 * Create a cgroup v1 with a device controller that does not limit
		 * access to any device. */
		sc_udev_make_unconfined(fds.devices_allow_fd);
	}
