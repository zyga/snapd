...
static void assert_caps(void) {
    cap_t __unsafe_indexable current SC_CLEANUP(cap_free) = cap_get_proc();
    ...
    for (size_t i = 0; i < SC_ARRAY_SIZE(expected_caps); i++) {
        ...
        const char *__unsafe_indexable cap_name SC_CLEANUP(cap_free) = cap_to_name(cap);
...
int main(int argc, char *__null_terminated *__counted_by(argc) argv) {
    ...
    sc_error *err = NULL;
    sc_instance_name_validate(snap_instance_name, (sc_error * __single * __single) & err);
    sc_die_on_error(err);
    ...
    const char *__null_terminated ns_dir_path = "/run/snapd/ns";
    ...
    DIR *__unsafe_indexable ns_dir = fdopendir(ns_dir_fd);
    if (ns_dir == NULL) {
    ...
    while (true) {
        ...
        errno = 0;
        struct dirent *__unsafe_indexable dent = readdir(ns_dir);
        if (dent == NULL) {
            if (errno != 0) {
        ...
        const char *__null_terminated dname = __unsafe_forge_null_terminated(const char *, dent->d_name);
        ...
        struct variant {
            const char *__null_terminated pattern;
            bool unmount;
        };
        struct variant variants[] = {
            {.pattern = __unsafe_forge_null_terminated(const char *, &sys_mnt_pattern[0]), .unmount = true},
            {.pattern = __unsafe_forge_null_terminated(const char *, &usr_mnt_pattern[0]), .unmount = true},
            {.pattern = __unsafe_forge_null_terminated(const char *, &sys_fstab_pattern[0])},
            {.pattern = __unsafe_forge_null_terminated(const char *, &usr_fstab_pattern[0])},
            {.pattern = __unsafe_forge_null_terminated(const char *, &sys_info_pattern[0])},
        };
