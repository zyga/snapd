...
func (v *VFS) propagateMount(m *mount, skipFn func(m *mount) bool) {
	...
	v.peersAndSlavesOf(m.parent.shared)(func(peer *mount) bool {
		...
		if peer.shared == m.parent.shared {
			// Propagating into a peer
			shared = m.shared
		} else {
			// Propagating into a slave.
			master = m.shared
			// The slave is also shared so the propagated mount joins a new peer group.
			if shared == 0 && peer.shared != 0 {
				shared = v.allocateGroupID()
			}
		}

		// TODO: skip if peer does not see m's directory.
		newM := &mount{
			// FIXME: This is wrong when parts of mount are bind-mounted.
