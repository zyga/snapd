...
func (s *nfsSuite) TestIsHomeUsingNFS(c *C) {
	...
	for _, tc := range cases {
		...
		comment := Commentf("mountinfo %#v", tc.mountinfo)

		nfsOrCifs, err := osutil.IsHomeUsingNFSorCIFS()
		if tc.errorPattern != "" {
			c.Assert(err, ErrorMatches, tc.errorPattern, comment)
		} else {
			c.Assert(err, IsNil)
		}
		c.Assert(nfsOrCifs, Equals, tc.nfsOrCifs, comment)
	}
}
